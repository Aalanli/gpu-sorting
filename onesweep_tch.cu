...
torch::Tensor onesweep_b32(torch::Tensor data, int64_t kernel) {
  TORCH_CHECK(data.device().is_cuda(), "data must be a CUDA tensor");
  TORCH_CHECK(data.scalar_type() == torch::kInt32,
              "data must be of type uint32");
  TORCH_CHECK(data.dim() == 1, "data must be a 1D tensor");
  const int N = data.size(0);

  TORCH_CHECK(0 <= kernel && kernel < kernels.size(),
              "kernel must be in range [0, ", kernels.size(), ")");
  auto onesweep = kernels[kernel];
  int workspace_size = onesweep->get_workspace_size_in_bytes(N);

  auto workspace = torch::empty(
      {workspace_size},
      torch::TensorOptions().dtype(torch::kBool).device(data.device()));

  auto res = torch::empty_like(data);

  onesweep->launch(reinterpret_cast<const uint32_t *>(data.data_ptr<int>()),
                   reinterpret_cast<uint32_t *>(res.data_ptr<int>()),
                   reinterpret_cast<uint8_t *>(workspace.data_ptr<bool>()), N,
                   c10::cuda::getCurrentCUDAStream().stream());

  return res;
}

int64_t num_kernels() { return kernels.size(); }
