...
__global__ void DigitBinningPassKeysOnly(const T *sort, T *alt,
  ...
  constexpr int SHARED_BYTES =
      KEY_DATA_SIZE * sizeof(T) < WARP_HIST_SIZE * sizeof(uint32_t)
          ? WARP_HIST_SIZE * sizeof(uint32_t)
          : KEY_DATA_SIZE * sizeof(T);
  ...
  // clear shared memory
  for (uint32_t i = threadIdx.x; i < WARP_HIST_SIZE;
  ...
  if (partitionIndex < gridDim.x - 1) {
    for (uint32_t i = 0, t = getLaneId() + (WARP_INDEX * 32 * ELEM_PER_THREAD) +
                             partitionIndex * THREADS * ELEM_PER_THREAD;
         i < ELEM_PER_THREAD; ++i, t += 32)
      keys[i] = sort[t];
  }
  ...
  for (uint32_t i = 0; i < ELEM_PER_THREAD; ++i) {
    ...
    if (bits == 0)
      preIncrementVal =
          atomicAdd((uint32_t *)&s_warpHist[keys[i] >> radixShift & RADIX_MASK],
                    __popc(warpFlags));
  ...
  if (threadIdx.x < RADIX) {
    ...
    atomicAdd(
        (uint32_t *)&passHistogram[threadIdx.x + (partitionIndex + 1) * RADIX],
  ...
  static_assert(THREADS >= RADIX,
                "THREADS must be greater than or equal to RADIX");
  if (threadIdx.x < RADIX) {
  ...
  if (partitionIndex < gridDim.x - 1) {
    for (uint32_t i = threadIdx.x; i < KEY_DATA_SIZE; i += blockDim.x)
      alt[s_localHistogram[s_key_data[i] >> radixShift & RADIX_MASK] + i] =
          s_key_data[i];
  }

  if (partitionIndex == gridDim.x - 1) {
    const uint32_t finalPartSize =
        size - partitionIndex * THREADS * ELEM_PER_THREAD;
...
__global__ void
compute_histograms_shared(const T *__restrict__ data,     // [N]
                          uint32_t *__restrict__ g_hists, // [HISTOGRAMS, RADIX]
                          const int N, const int radix_stride) {
  constexpr int RADIX = 1 << RADIX_LOG;
  constexpr int RADIX_MASK = RADIX - 1;
  ...
  for (int i = threadIdx.x + blockIdx.x * THREADS; i < N;
       i += THREADS * gridDim.x) {
    T dreg = (i < N) ? data[i] : 0;
...
__global__ void compute_histograms_shared_pipelined(
  ...
  __shared__ uint32_t s_hists[RADIX_DUP][HISTOGRAMS * RADIX];
  cuda::pipeline<cuda::thread_scope::thread_scope_thread> pipeline =
      cuda::make_pipeline();
  ...
  for (int i = 0; i < NPIPE; ++i) {
    const int offset =
        threadIdx.x + blockIdx.x * THREADS + i * THREADS * gridDim.x;
    pipeline.producer_acquire();
    cuda::memcpy_async(&s_data[i * THREADS + threadIdx.x], &data[offset],
                       (offset < N) ? sizeof(T) : 0, pipeline);
  ...
  for (int i = NPIPE; i < iters; i++) {
    const int offset =
        threadIdx.x + blockIdx.x * THREADS + i * THREADS * gridDim.x;
    const int offset_prev =
        threadIdx.x + blockIdx.x * THREADS + (i - NPIPE) * THREADS * gridDim.x;
    ...
    pipeline.producer_acquire();
    cuda::memcpy_async(&s_data[((i - NPIPE) % NPIPE) * THREADS + threadIdx.x],
                       &data[offset], (offset < N) ? sizeof(T) : 0, pipeline);
    pipeline.producer_commit();
  }
...
void launch_compute_histograms(const T *data,     // [N]
  ...
  if (N < 2500000) {
    compute_histograms_shared<T, HISTOGRAMS, RADIX_LOG, 1>
        <<<blocks, threads, 0, stream>>>(data, g_hists, N, radix_stride);
  } else {
    const int smem_dynamic = threads * sizeof(T) * 3;
    compute_histograms_shared_pipelined<T, HISTOGRAMS, RADIX_LOG, 1, 3>
        <<<blocks, threads, smem_dynamic, stream>>>(data, g_hists, N,
                                                    radix_stride);
  }
}
...
//     const int blocks = divup(N, threads * reps_per_thread);
//     if (N < 2500000) {
//         compute_histograms_shared<uint32_t, HISTOGRAMS, RADIX_LOG,
//         1><<<blocks, threads, 0, stream>>>(
//             data, g_hists, N, radix_stride
//         );
//     } else {
//         const int smem_dynamic = threads * sizeof(uint32_t) * 3;
//         compute_histograms_shared_pipelined<uint32_t, HISTOGRAMS, RADIX_LOG,
//         1, 3><<<blocks, threads, smem_dynamic, stream>>>(
//             data, g_hists, N, radix_stride
//         );
//     }
// }

template <typename T> void print_device_mem(const T *data, int N) {
  T *h_data = new T[N];
...
template <typename T> class AbstractLauncher {
public:
  virtual int get_workspace_size_in_bytes(int N) = 0;
  virtual void launch(const T *data,      // [N]
                      T *out,             // empty[N]
                      uint8_t *workspace, // empty[workspace_size]
                      const int N, CUstream_st *stream) = 0;
};

template <typename T, int SORTING_BITS, int RADIX_LOG, int THREADS,
          int ELEM_PER_THREAD>
class OneSweepLauncher : public AbstractLauncher<T> {
  static int get_blocks(int N) { return divup(N, THREADS * ELEM_PER_THREAD); }

  static int get_radix_stride(int blocks) {
    return (1 << RADIX_LOG) * (blocks + 1);
  }

  static int radix() { return 1 << RADIX_LOG; }
...
public:
  ...
  void launch(const T *data,      // [N]
              T *out,             // empty[N]
              uint8_t *workspace, // empty[workspace_size]
              const int N, CUstream_st *stream) override {
    const int blocks = get_blocks(N);
    const int radix_stride = get_radix_stride(blocks);
    ...
    auto kernel =
        DigitBinningPassKeysOnly<T, RADIX_LOG, ELEM_PER_THREAD, THREADS>;
    T *src;
    T *dst;
    ...
    for (int i = 0; i < radix_iters(); ++i) {
      ...
      kernel<<<blocks, THREADS, 0, stream>>>(
          (const T *)src, dst, (volatile uint32_t *)g_hists + radix_stride * i,
          (volatile uint32_t *)index + i, N, radix_shift);
