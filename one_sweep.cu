...
// template <typename T, int ELEM_PER_THREAD, int THREADS>
// __device__ void DigitBinningPassKeysOnlyTemplate(
//     const T* sort,
//     T* alt,
...
//     __align__(4) uint8_t s_data[SHARED_BYTES];
...
//     //atomically assign partition tiles
//     if (threadIdx.x == 0)
//         s_warpHistograms[WARP_HIST_SIZE - 1] = atomicAdd((uint32_t*)&index[radixShift >> 3], 1);
//     __syncthreads();
...
//     DigitBinningPassKeysOnlyTemplate<uint32_t, 15, 512>(sort, alt, (uint32_t*)(passHistogram), (uint32_t*) index, size, radixShift);
// }


__global__ void OneSweep::DigitBinningPassPairs(
    uint32_t* sort,
